...
setInterval( async() => {
    ...
    if(timeCheck.getHours() === 18 && timeCheck.getMinutes() >= 6 && timeCheck.getMinutes() <= 6 && checkSendLine) {
         try {
            const pool = await poolPromise;
            const result3 = await pool.request().query(`
            SELECT	[WT-01]AS WT1, [WT-02]AS WT2, [WT-03]AS WT3, [WT-04]AS WT4, [WT-05]AS WT5, [WT-06]AS WT6,
            ...
            if(result3) {
                ...
                await lineNotify.notify({
                            message: `วันที่ ${timeCheck.toLocaleString()}
